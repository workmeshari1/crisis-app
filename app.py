...
# ========== تعطيل النقر الأيمن والاختصارات ==========
st.markdown("""
<script>
...
def load_data_and_password():
    try:
        ...
        if not creds_json or not sheet_id:
            raise ValueError("❌ بيانات Google Sheet مفقودة. تأكد من إعداد المتغيرات البيئية.")
        ...
        st.error(f"❌ فشل تحميل البيانات من Google Sheets: {str(e)}")
        # استخدام البيانات الاختبارية كبديل
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            for syn in synonyms.split(","):
                if is_number_in_range(number, syn.strip()):
                    st.markdown(f"""
...
for col in [DESC_COL, ACTION_COL]:
    if col not in df.columns:
        st.error(f"❌ عمود مفقود: '{col}'. تأكد من الأعمدة في Google Sheet.")
...
if not st.session_state.authenticated:
    ...
    if not PASSWORD:
        st.error("❌ لم يتم العثور على كلمة المرور في الخلية F1 من Google Sheet")
        st.stop()
...
query = st.text_input("ابحث هنا:", placeholder="اكتب وصف الحالة أو رقم...")
if not query:
    st.stop()
    ...
    if st.button("🤖 البحث الذكي", use_container_width=True):
        try:
            with st.spinner("جاري البحث الذكي..."):
                ...
                if not found:
                    st.info("🤖 لم يتم العثور على نتائج مشابهة كافية. حاول إعادة صياغة السؤال.")
